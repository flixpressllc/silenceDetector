...
namespace FlixpressFFMPEG.SilenceDetector
{
    public static class AudibleDetectorExecutor
    {
        private static void ExtractAudible(string ffmpegExecutablePath, TimeInterval timeIntervalToKeep, string inputFilename, string outputFilename)
        {
            ...
            ffmpeg.StartInfo.Arguments = @"-i " + inputFilename + " -ss " + timeIntervalToKeep.Start + " -c:v libx264 -crf 0 -preset ultrafast -c:a copy -y -to " + (timeIntervalToKeep.End) + " " + outputFilename;
        ...
        {   
            ...
            List<TimeInterval> audibleTimeIntervals = Helpers.ExtractAudibleTimeIntervals(output, minimumSilenceIntervalToCut, audibleClipSilencePadding, includeSilenceIntervals);
